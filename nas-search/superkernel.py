...
class DepthwiseConv2DMasked(tf.keras.layers.DepthwiseConv2D):
    def __init__(self,
        ...
        self.runtimes = runtimes  # Runtime情報がここで渡されている。
        ...
        if kernel_size[0] != 5:  # normal Depthwise type
            ...
            if self.runtimes is not None:
                ...
                self.R100c = K.cast_to_floatx(
                    self.runtimes[3])  # 100% of the 5x5
    ...
    def build(self, input_shape):
        ...
        if not self.custom:
            ...
            # you cannot mask out if it is 3x3 already!
            assert kernel_shape[0] == 5
            # you cannot mask out if it is 3x3 already!
            assert kernel_shape[1] == 5

            # Thresholds
            self.t5x5 = self.add_weight(
                shape=(1,), initializer='zeros', name="t5x5")
            self.t50c = self.add_weight(
                shape=(1,), initializer='zeros', name="t50c")
            self.t100c = self.add_weight(
                shape=(1,), initializer='zeros', name="t100c")
            ...
            if self.strides[0] == 1 and len(self.runtimes) == 5:
                ...
                if self.dropout_rate is not None:  # noise to add
                    self.d50c = tf.nn.dropout(
                        Indicator(x50c), self.dropout_rate)
                else:
            ...
            if self.runtimes is not None:
                ...
                runtime_channels = self.d50c * \
                    (self.R50c + self.d100c * (self.R100c-self.R50c))
                runtime = runtime_channels * ratio + \
                    runtime_channels * (1-ratio) * self.d5x5
