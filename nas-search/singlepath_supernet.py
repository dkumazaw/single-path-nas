...
def dense_kernel_initializer(shape, dtype=None, partition_info=None):
    ...
    Returns:
      an initialization for the variable
    """
    # Denseの方のInitializationもWrapしてある。
    del partition_info
...
class MBConvBlock(object):
    ...
    """
    # こいつが複数個スタックされることでネットワークが構成されている。
    # MobilenetV3対応のためにはここを書き換えれば良いと思う
    ...
    def _build(self):
        ...
        if self._block_args.expand_ratio != 1:
            # Expansion phase: Dimensionを上げている
        ...
        # Learnable Depth-wise convolution Superkernel
            ...
            raise NotImplementedError(
                'DepthConv not defined for %s' % self._search_space)
        ...
        if self.has_se:
            ...
            # Squeeze and excitationはサーチ後に再度Enableしてやってもよいよね！！
            assert 1 == 0
        ...
        # Output phase: Down scaleしてDimensionをもとに戻してやる
    ...
    def call(self, inputs, training=True):
        ...
        for idx, block in enumerate(self._blocks):
            with tf.variable_scope('mnas_blocks_%s' % idx):
                outputs, total_runtime = block.call(
                    outputs, total_runtime, training=training)
